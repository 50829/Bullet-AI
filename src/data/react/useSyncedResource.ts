...
import { useDataResource, useDataRuntime } from "./provider";
...
import { loadRemoteChangeWatermark } from "../supabase/change-reader";
import {
...
import { synchronizeRemoteResource } from "../supabase/remote-sync";
...
export function useSyncedResource<R extends DataResource>(
  userId: string | null,
  resource: R,
  options: { fullHistory?: boolean } = {},
) {
  const { notifier, store } = useDataRuntime();
