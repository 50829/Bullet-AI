...
import type { RemoteMutationRequest } from "../local";
...
function legacyProfileCreate(): RemoteMutationRequest<"profiles"> {
  return {
    mutationId: "mutation-profile",
    userId: "user-1",
    resource: "profiles",
    clientId: "user-1",
    kind: "create",
...
describe("SupabaseRemoteMutationExecutor idempotency", () => {
  ...
  it("turns a legacy profile create into a conflict instead of losing local changes", async () => {
    ...
    await expect(
      ...
      kind: "conflict",
      remote: { username: "Cloud name", version: 3 },
    });
