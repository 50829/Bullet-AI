...
export class MutationMaintenanceRepository {
  ...
  async getDiagnostics(userId: string): Promise<DataDiagnostics> {
  ...
  async retryBlocked(mutationId: string) {
    const db = await this.dependencies.database.open();
    ...
    if (
      ...
      mutation.blockedReason !== "permanent"
    ) {
      await transaction.done;
      return false;
    }
    const now = this.dependencies.now();
    const queued: AnyMutationRecord = {
      ...mutation,
      status: "queued",
      attemptCount: 0,
      nextAttemptAt: now,
    ...
    await transaction.done;
    notifyMutation(this.dependencies, queued, "mutation-changed");
    return true;
  }
