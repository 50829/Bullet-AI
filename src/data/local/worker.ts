...
import { DATA_WEB_LOCK_PREFIX } from "../protocol";
import type { DataStoreApi } from "./store";
import type {
  AnyMutationRecord,
  DataResource,
  EntityByResource,
...
export class DataSyncWorker {
  ...
  private async flushWithLock() {
    ...
    await locks.request(
      this.options.lockName ?? `${DATA_WEB_LOCK_PREFIX}:${this.options.userId}`,
      { ifAvailable: true },
      async (lock) => {
  ...
  private async executeMutation(mutation: AnyMutationRecord) {
    ...
    } as RemoteMutationRequest<DataResource>;
