...
import type { MutationRecord } from "../../../lib/data-v2/types";
import {
...
describe("workspace history pagination", () => {
  ...
  it("keeps remote history pages out of durable snapshot storage", () => {
    const source = readFileSync(
      new URL("./useWorkspaceResourceV2.ts", import.meta.url),
      "utf8",
    );
    expect(source).not.toContain("store.putSnapshot");
    expect(source).not.toContain("store.replaceSnapshots");
  });
  ...
  it("wires both history pages to a localized, disabled-while-loading button", () => {
    const momentsSource = readFileSync(
      new URL("../../../app/moments/MomentsPageClient.tsx", import.meta.url),
      "utf8",
    );
    const reflectionsSource = readFileSync(
      new URL(
        "../../../app/reflections/ReflectionsPageClient.tsx",
        import.meta.url,
      ),
      "utf8",
    );
    ...
    const chinese = JSON.parse(
      readFileSync(
        new URL("../../../../messages/zh.json", import.meta.url),
        "utf8",
      ),
    ) as Record<string, string>;

    for (const source of [momentsSource, reflectionsSource]) {
      expect(source).toContain("{hasMore && (");
      expect(source).toContain("disabled={loadingMore}");
      expect(source).toContain("onClick={() => void loadMore()}");
      expect(source).toContain('t("loadEarlierRecords"');
    }
    expect(english.loadEarlierRecords).toBe("Load earlier records");
    expect(chinese.loadEarlierRecords).toBe("加载更早记录");
  });
});
