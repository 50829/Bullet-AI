...
const LOGOUT_EPOCH_STORAGE_PREFIX = "bullet-ai:data-v2:logout-epoch:";
const LOGOUT_CLEANUP_STORAGE_PREFIX = "bullet-ai:data-v2:logout-cleanup:";
...
function logoutEpochKey(userId: string) {
  return `${LOGOUT_EPOCH_STORAGE_PREFIX}${userId}`;
}

function logoutCleanupKey(userId: string) {
  return `${LOGOUT_CLEANUP_STORAGE_PREFIX}${userId}`;
}
