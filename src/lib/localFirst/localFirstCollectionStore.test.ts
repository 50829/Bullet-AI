...
describe("LocalFirstCollectionStore", () => {
  ...
  it("uses a complete initial snapshot before hydrating the repository", async () => {
    const remote = moment({
      id: 1,
      client_id: "moment-1",
    ...
    expect(store.getServerSnapshot().items).toEqual([
      expect.objectContaining({ content: "initial remote" }),
    ]);
    expect(store.getSnapshot().items).toEqual([
      expect.objectContaining({ content: "initial remote" }),
    ]);
    ...
    expect(store.getSnapshot()).toMatchObject({
      loading: false,
      items: [expect.objectContaining({ content: "initial remote" })],
    });
  });

  it("keeps pending local rows alongside an incomplete initial page", async () => {
    const pending = moment({
      id: 99,
      client_id: "moment-pending",
      content: "pending",
    ...
    mocks.repository.cacheRemote.mockResolvedValue([pending, remote]);
    const store = LocalFirstCollectionStore.create<TestRecord>({
      collection: "moments",
      initialRemotePageSize: 2,
      initialSnapshot: {
    ...
    expect(store.getSnapshot()).toMatchObject({
      ...
      items: [
        expect.objectContaining({ content: "pending" }),
        expect.objectContaining({ content: "initial page" }),
      ],
    });
  });
});
