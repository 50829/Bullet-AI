...
本文记录代码审计发现的潜在一致性问题。它们是可以从当前并发模型推导出的失败路径，不代表已经观察到生产数据丢失。当前工作区包含部分未完成修复，在全量应用测试和本地数据库契约测试通过前，不应视为可发布状态。
...
只修改 `000` 或 `005` 不能修复已经执行过这些迁移的项目。权限收敛必须出现在新的前向迁移中。另外，文档声明的新库 `000 + 006...` 与旧库 `005 + 006...` 两条路径都需要在 CI 中实际执行，而不能只校验 manifest。

影响：旧部署可能保留匿名 RPC/trigger function 权限，或者升级路径在发布时才失败。

当前状态：权限修复已移动到 `006`，双路径本地 fixture 和脚本已有草拟实现，尚未运行最终数据库契约测试。
...
10. 最后只运行一次受控的本地 Supabase 验证：脚本拒绝复用已运行的栈、禁用 telemetry、用 trap 停止自己启动的栈，并分别验证 fresh 与 legacy 两条迁移路径。禁止使用 `supabase link` 或任何线上 project ref。

## 完成标准

- 针对 R1-R9 的回归测试通过。
- `pnpm migration:check`、typecheck、lint、format、全量 Vitest 和 production build 通过。
- 本地 fresh/legacy pgTAP 均通过，测试结束后没有 Supabase Docker 容器残留。
