...
- `src/features/workspace`: route-scoped composition, Supabase mapping, export, and sync diagnostics.
...
4. When online, the Supabase repository maps snake_case rows to domain entities in the background.
5. Version-aware replacement rejects stale reads that would overwrite a newer synced snapshot.
6. TanStack Query caches the overlay and BroadcastChannel invalidates it across tabs.

An empty collection has its own durable sync marker; absence of rows is not confused with a resource that has never loaded. Each authenticated browser session also has a local generation token, so callbacks started before logout cannot write into a later login for the same user.

Moments and reflections keep a 90-day durable recent set. Their dedicated pages make one full-history request, retain the full result only in memory, and refresh the recent snapshot subset. Goals and check-ins load complete history because retention settings and streak calculations require it.
...
- Fresh databases use `000_current_schema.sql`.
- Existing legacy databases require backup, preflight, then `005_domain_schema_v2.sql`.
